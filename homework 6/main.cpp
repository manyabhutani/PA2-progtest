...
class CSelfMatch
{
  public:
    ...
    CSelfMatch(const Container & container) : data(std::begin(container) , std::end(container)){}
    ...
    template<size_t N_>std::vector<std::array<size_t, N_>> findSequences() const {
        ...
        if (N_ == 0) throw std::invalid_argument("Invalid Input.\n");

        size_t longestLen = sequenceLen(N_);
        if (longestLen == 0) return result;

        std::vector<std::pair<std::vector<T_>, std::vector<size_t>>> subseqPositions;
        for (size_t start = 0; start + longestLen <= data.size(); ++start) {
            std::vector<T_> subseq(data.begin() + start, data.begin() + start + longestLen);
            bool found = false;
            for (auto& [existingSubseq, positions] : subseqPositions) {
                if (existingSubseq.size() == subseq.size() && std::equal(existingSubseq.begin(), existingSubseq.end(), subseq.begin())) {
            ...
            if (!found) {
                subseqPositions.emplace_back(subseq, std::vector<size_t>{start});
            }
        }

        for (const auto& [subseq, positions] : subseqPositions) {
            if (positions.size() >= N_) {
                std::vector<size_t> combination;
                combine(result, positions, combination, 0, 0, N_);
            }
        }
...
private:
    template<size_t N_>
    void combine(std::vector<std::array<size_t, N_>>& result, const std::vector<size_t>& positions, std::vector<size_t>& combination,size_t start,size_t len,size_t max_len) const {
        if (len == max_len) {
            std::array<size_t, N_> arr;
            std::copy_n(combination.begin(), max_len, arr.begin());
            result.push_back(arr);
            return;
        }

        for (size_t i = start; i <= positions.size() - (max_len - len); ++i) {
            combination.push_back(positions[i]);
            combine(result, positions, combination, i + 1, len + 1, max_len);
...
int main ()
{
   CSelfMatch<char> m1("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"s);
   std::cout <<((m1.sequenceLen(5)));
